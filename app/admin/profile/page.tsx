    ...
    return (
        <div className="max-w-4xl mx-auto space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <Link 
                ...
                className="inline-flex items-center gap-2 text-[10px] font-medium text-gray-400 uppercase tracking-[0.2em] hover:text-black transition-colors"
            >
                <ArrowLeft size={14} />
                Back to Dashboard
            </Link>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="bg-white p-10 rounded-[2.5rem] border border-gray-100  space-y-8">
                    <div className="flex items-center gap-4">
                        ...
                        <h2 className="text-2xl font-medium text-black tracking-tight">Account Information</h2>
                    </div>

                    <div className="space-y-6">
                        <div className="p-6 bg-gray-50 rounded-2xl border border-gray-100 group hover:border-[#D60000]/20 transition-colors">
                            <p className="text-[10px] font-medium text-gray-400 uppercase tracking-widest mb-2">Full Name</p>
                            <p className="font-medium text-black text-lg">{displayName}</p>
                        </div>

                        <div className="p-6 bg-gray-50 rounded-2xl border border-gray-100 group hover:border-[#D60000]/20 transition-colors">
                            <p className="text-[10px] font-medium text-gray-400 uppercase tracking-widest mb-2">Phone Number</p>
                            <p className="font-medium text-black text-lg">{user?.phone || 'Not linked'}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white p-10 rounded-[2.5rem] border border-gray-100  space-y-8">
                    <div className="flex items-center gap-4">
                        ...
                        <h2 className="text-2xl font-medium text-black tracking-tight">Security & Access</h2>
                    </div>

                    <div className="space-y-6">
                        <div className="p-6 bg-gray-50 rounded-2xl border border-gray-100">
                            <p className="text-[10px] font-medium text-gray-400 uppercase tracking-widest mb-2">Account Role</p>
                            <div className="flex items-center gap-3">
                                <span className="w-2 h-2 bg-green-500 rounded-full" />
                                <p className="font-medium text-black text-lg capitalize">{user?.role || 'User'}</p>
                            </div>
                        </div>

                        <div className="p-6 bg-gray-50 rounded-2xl border border-gray-100">
                            <p className="text-[10px] font-medium text-gray-400 uppercase tracking-widest mb-2">Store Assignment</p>
                            <div className="flex items-center gap-3">
                                <Store size={18} className="text-gray-400" />
                                <p className="font-medium text-black text-lg">{user?.store_id ? 'Central Supermarket' : 'Standalone'}</p>
                            </div>
                        </div>
                    </div>
                </div>
