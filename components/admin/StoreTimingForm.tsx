                ...
                {DAYS.map((day) => (
                    <div key={day} className="flex flex-col md:flex-row md:items-center justify-between p-6 bg-gray-50/50 rounded-[2rem] border border-gray-100 hover:border-gray-200 transition-all group gap-4">
                        <div className="flex items-center gap-6">
                            ...
                            />
                            <span className="font-medium text-black min-w-[100px]">{day}</span>
                            {!localTiming[day].enabled && (
                                <span className="text-[10px] font-medium text-[#D60000] uppercase tracking-widest bg-red-50 px-3 py-1 rounded-full">Closed</span>
                            )}
                        </div>
                        ...
                        <div className={cn("flex items-center gap-4 transition-opacity duration-300", !localTiming[day].enabled && "opacity-20 pointer-events-none")}>
                            <div className="space-y-1">
                                <p className="text-[10px] font-medium text-gray-400 uppercase tracking-widest text-center">Open</p>
                                <select
                                    value={localTiming[day].open}
                                    onChange={e => updateDay(day, { open: e.target.value })}
                                    className="bg-white border border-gray-100 rounded-xl px-4 py-2 text-xs font-medium outline-none focus:border-black transition-all"
                                >
                            ...
                            <div className="space-y-1">
                                <p className="text-[10px] font-medium text-gray-400 uppercase tracking-widest text-center">Close</p>
                                <select
