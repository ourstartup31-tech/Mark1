                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-6">
                    {categories.map((cat, i) => (
                        <AnimatedSection key={cat.id} delay={i * 60}>
                            <button className="w-full group bg-white border border-gray-100 rounded-2xl p-6 text-center hover:border-black hover:shadow-xl hover:shadow-gray-200/40 hover:-translate-y-1 transition-all duration-300 cursor-pointer">
                                <div className="text-3xl mb-4 group-hover:scale-110 transition-transform duration-300">
                                    {cat.emoji}
                                </div>
                                <p className="font-bold text-sm text-black leading-tight mb-1">
                                    {cat.name}
                                </p>
